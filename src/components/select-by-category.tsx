...
import { ChevronLeft, ChevronRight, Smartphone, Heart, Headphones, Shirt, Home, Car, Book, Gamepad2, Laptop, Watch } from "lucide-react"
import { useRouter } from "next/navigation"
                            ...
                            <div className="flex items-center space-x-2 py-4 min-w-max">
                                {categories.map((category, index) => {
                                    ...
                                    return (
                                        <button
                                            key={category.id}
                                            onClick={(e) => handleSearchProductWithCategory(e, category.id)}
                                            onMouseEnter={() => setHoveredCategory(category.id)}
                                            onMouseLeave={() => setHoveredCategory(null)}
                                            className={`
                                                group relative flex flex-col items-center justify-center
                                                px-4 py-3 min-w-[120px] rounded-xl
                                        ...
                                        >
                                            ...
                                            <div className={`
                                                ...
                                                bg-gradient-to-r ${category.gradient} transition-opacity duration-300
                                            `}></div>
                                            
                                            {/* Icon Container */}
                                            <div className={`
                                                relative p-2 rounded-lg ${category.bgColor} mb-2
                                            ...
                                            <div className={`
                                                ...
                                                w-0 h-1 bg-gradient-to-r ${category.gradient} rounded-full
                                                transition-all duration-300
                                                ${hoveredCategory === category.id ? 'w-8' : 'group-hover:w-4'}
                                            `}></div>
                                        </button>
                                    )
                                })}
